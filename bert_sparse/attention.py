...
if __name__ == '__main__':
    ...
    print('full_attn_tf time taken:\t%8.5f' % (time.perf_counter() - start))

    start = time.perf_counter()
    full_attn_bs = blocksparse_attention_impl(q, k, v, heads=4, attn_mode="all", blocksize=blocksize, recompute=True)
    print('full_attn_bs time taken:\t%8.5f' % (time.perf_counter() - start))
    ...
    print('local_attn_bs time taken:\t%8.5f' % (time.perf_counter() - start))

    start = time.perf_counter()
    local_attn_tf = attention_impl(q, k, v, heads=4, attn_mode="local", local_attn_ctx=32, recompute=True)
    print('local_attn_tf time taken:\t%8.5f' % (time.perf_counter() - start))
    ...
    print('strided_attn_bs time taken:\t%8.5f' % (time.perf_counter() - start))

    start = time.perf_counter()
    strided_attn_tf = attention_impl(q, k, v, heads=4, attn_mode="strided", local_attn_ctx=32, recompute=True)
    print('strided_attn_tf time taken:\t%8.5f' % (time.perf_counter() - start))
